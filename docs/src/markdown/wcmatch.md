...
#### `wcmatch.SORT, wcmatch.T` {: #wcmatchsort}

`SORT` will cause the returned files to be sorted. Files are sorted alphabetically using Python's default sort.

```pycon3
>>> from wcmatch import wcmatch
>>> wcmatch.WcMatch('.', '*').match()
['./appveyor.yml', './LICENSE.md', './MANIFEST.in', './mkdocs.yml', './README.md', './setup.cfg', './setup.py', './tox.ini']
>>> wcmatch.WcMatch('.', '*', flags=wcmatch.SORT).match()
['./LICENSE.md', './MANIFEST.in', './README.md', './appveyor.yml', './mkdocs.yml', './setup.cfg', './setup.py', './tox.ini']
```

In [`glob`](./glob.md), sorting is done per pattern, but due to how file crawling is handled in `WcMatch`, results are
sorted across all patterns. Patterns separated by `|` and patterns generated by [`BRACE`](#wcmatchbrace) will return
the entire list of files sorted. As shown below, the results of `*.yml` and `*.md` are all sorted together.

```pycon3
>>> from wcmatch import glob
>>> wcmatch.WcMatch('.', '*.yml|*.md', flags=wcmatch.SORT).match()
['./LICENSE.md', './README.md', './appveyor.yml', './mkdocs.yml']
>>> wcmatch.WcMatch('.', '{*.yml,*.md}', flags=wcmatch.BRACE | wcmatch.SORT).match()
['./LICENSE.md', './README.md', './appveyor.yml', './mkdocs.yml']
```
